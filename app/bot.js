...
const BOT_MODE = process.env.TELEGRAM_BOT_MODE || process.env.BOT_MODE || 'webhook';
...
if (BOT_MODE === 'polling') {
  ...
  // Ensure webhook is set once in webhook mode
  const WEBHOOK_URL = process.env.TELEGRAM_WEBHOOK_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}/api/webhook` : undefined);
  const WEBHOOK_SECRET = process.env.TELEGRAM_WEBHOOK_SECRET || process.env.WEBHOOK_SECRET;
  if (WEBHOOK_URL) {
    const options = {};
    if (WEBHOOK_SECRET) {
      options.secret_token = WEBHOOK_SECRET;
    }
    bot.setWebHook(WEBHOOK_URL, options).catch(() => {});
  }
}

// Export bot instance for use in API route
module.exports = { bot };
...
async function checkUserRegistration(chatId, telegramId, firstName, lastName, fromStart = false) {
  try {
    ...
    if (!supabaseUrl || !serviceKey) {
      console.warn('Supabase env missing. NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set.');
      bot.sendMessage(chatId, 
        `Halo ${firstName}! 👋\n\n` +
        'Selamat datang di Order Management Bot!\n\n' +
        'Saat ini konfigurasi database belum lengkap.\n' +
...
//coba buat menu SOD order
  ...
  showSODOrder(chatID, telegramId);
      ...
      const role = await getUserRole(telegramId);
      showWelcomeMessage(chatId, role, 'User');
